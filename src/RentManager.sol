...
/// @notice RentManager is in charge of facilitating the rent / storage logic of certain TNFT assets.
contract RentManager is AdminAccess, IRentManager {

    // ~ State Variabls ~

    mapping(address => bool) public override registered;

    mapping(address => bool) public override paysRent; // TODO: What is this for?
    ...
    function registerWithRentManager(address _contract, bool _paysForStorage) external override onlyFactoryAdmin {
    ...
    function adjustStorageAndGetAmount(address _contract, uint256 tokenId, uint256 _years, uint256 tokenPrice) external override onlyFactory returns (uint256) {
        require(registered[_contract], "RentManager.sol::adjustStorageAndGetAmount() contract provided is not registered");
        ...
        if (storagePriceFixed[_contract]) {
            ...
            require(tokenPrice > 0, "RentManager.sol::adjustStorageAndGetAmount() tokenPrice == 0");
    ...
    function setStoragePricePerYear(address _contract, uint256 _storagePricePerYear) external override onlyFactoryAdmin {
        require(registered[_contract], "RentManager.sol::setStoragePricePerYear() contract provided is not registered");
        require(_storagePricePerYear >= 1000000, "RentManager.sol::setStoragePricePerYear() storagePricePerYear must be gt 1000000 ($1)");
    ...
    function setStoragePercentPricePerYear(address _contract, uint256 _storagePercentagePricePerYear) external override onlyFactoryAdmin {
        require(registered[_contract], "RentManager.sol::setStoragePercentPricePerYear() contract provided is not registered");
        require(_storagePercentagePricePerYear >= 50, "RentManager.sol::setStoragePercentPricePerYear() storagePercentagePerYear must be gt 50 (0.5%)");
    ...
    function toggleStorageFee(address _contract, bool value) external onlyFactoryAdmin {
        require(registered[_contract], "RentManager.sol::toggleStorageFee() contract provided is not registered");
